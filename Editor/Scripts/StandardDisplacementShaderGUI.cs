...
#if UNITY_EDITOR
using UnityEditor;
namespace StandardDisplacement
{
	public class StandardDisplacementShaderGUI : ShaderGUI
	{
		...
		private static class Styles
		{
			...
			public static GUIContent displacementMapText = EditorGUIUtility.TrTextContent("Displacement Map", "Displacement Map");
			public static GUIContent tessellationFactorText = EditorGUIUtility.TrTextContent("Tessellation factor", "Tessellation factor");
			public static GUIContent tessellationDistanceText = EditorGUIUtility.TrTextContent("Tessellation distance", "Tessellation distance");


			public static string primaryMapsText = "Main Maps";
			public static string secondaryMapsText = "Secondary Maps";
			public static string displacementSettingsText = "Displacement Settings";
		...
		MaterialProperty displacementMap = null;
		MaterialProperty displacementScale = null;
		MaterialProperty tessellationFactor = null;
		MaterialProperty tessellationDistance = null;
		...
		public void FindProperties(MaterialProperty[] props)
		{
			...
			displacementMap = FindProperty("_DisplacementMap", props);
			displacementScale = FindProperty("_DisplacementScale", props);
			tessellationFactor = FindProperty("_TessellationFactor", props);
			tessellationDistance = FindProperty("_TessellationDistance", props);
		}
		...
		void DoDisplacementArea()
		{
			GUILayout.Label(Styles.displacementSettingsText, EditorStyles.boldLabel);
			m_MaterialEditor.TexturePropertySingleLine(Styles.displacementMapText, displacementMap, displacementMap.textureValue != null ? displacementScale : null);
			if (displacementScale.floatValue != 1
				&& UnityEditorInternal.InternalEditorUtility.IsMobilePlatform(EditorUserBuildSettings.activeBuildTarget))
				if (m_MaterialEditor.HelpBoxWithButton(
					EditorGUIUtility.TrTextContent("Displacement scale is not supported on mobile platforms"),
					EditorGUIUtility.TrTextContent("Fix Now")))
				{
					displacementScale.floatValue = 1;
				}

			m_MaterialEditor.ShaderProperty(tessellationFactor, Styles.tessellationFactorText);
			MinMaxShaderProperty(m_MaterialEditor, tessellationDistance, 0, 100, Styles.tessellationDistanceText);
		}
